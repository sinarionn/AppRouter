...
open class ARConfiguration<T: UIViewController> {
    ...
    open func push(animated: Bool, completion: (()->())? = nil) throws -> T {
        ...
        guard !(embedded.parent is UINavigationController) else { throw ARConfigurationErrors.tryingToPushNavigationController }
        ...
        let targetNavigation = try targetController as? UINavigationController ??
            ...
            ARConfigurationErrors.failedToFindNavigationControllerToPushOn.rethrow()
    ...
    /// - returns: returns instance provided by `source` provider
    @discardableResult
    open func setAsRoot(animator: (AppRouterType, UIViewController, ((Bool)->())?) throws -> (),
        completion: ((Bool)->())? = nil) throws -> T
    {
        let embedded = try provideEmbeddedSourceController()
        try animator(router, embedded.parent, completion)
        return embedded.child
    }
    ...
    open func setAsRoot() throws -> T {
        return try setAsRoot(animator: ARAnimators.SimpleSetRootAnimator().animate)
    }
    ...
    open func performTargetConstruction<U: UIViewController>() throws -> U {
        return try targetProvider() as? U ?? ARConfigurationErrors.failedToConstructTargetController.rethrow()
    }
...
public enum ARConfigurationErrors: LocalizedError {
...
public enum PresentationTarget {
    ...
    public func provideController<T: UIViewController>() throws -> T {
        switch self {
        case .top(let router):
            return try router.topViewController as? T ?? ARConfigurationErrors.failedToConstructTargetController.rethrow()
        case .root(let router):
            return try router.rootViewController as? T ?? ARConfigurationErrors.failedToConstructTargetController.rethrow()
        case .anonymous(let provider):
            return try provider() as? T ?? ARConfigurationErrors.failedToConstructTargetController.rethrow()
        }
    }
}
...
public enum PresentationSource {
    ...
    public func provideController<T: UIViewController>() throws -> T {
        ...
        case .storyboard(let initial):
            return try T.instantiate(initial: initial) ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        case .customStoryboard(let name, let initial):
            return try T.instantiate(storyboardName: name, initial: initial) ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        case .xib:
            return try T.instantiateFromXib() ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        case .customXib(let name):
            return try T.instantiateFromXib(name) ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        case .preconstructed(let vc):
            return try vc as? T ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        case .anonymous(let provider):
            return try provider() as? T ?? ARConfigurationErrors.failedToConstructSourceController.rethrow()
        }
    }
}
